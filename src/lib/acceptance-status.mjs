...
export const LIVE_ITEMS = Object.freeze([...range(101, 109), ...range(201, 205), ...range(301, 303), ...range(305, 310), ...range(321, 324), ...range(401, 405), ...range(501, 508), "601", ...range(603, 605), ...range(607, 608), ...range(701, 703), ...range(708, 717), ...range(801, 838), ...range(901, 905)]);
    ...
    return table.replace("<th>Item</th>", '<th scope="col" class="acceptance-status-heading">Current Status</th><th>Item</th>')
      ...
      .replace(/<tr>\s*<td>(\d{3})<\/td>([\s\S]*?)<\/tr>/g, (_row, item, rest) => {
        ...
        if (retiredItems.has(item) && cells[0] === "Retired") {
          retiredCount++;
          return ""; // Omit retired rows from current output, not source or historical evidence.
        }
