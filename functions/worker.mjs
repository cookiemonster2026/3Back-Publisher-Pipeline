...
async function handleDomainGuides(request, env) {
	...
	const form = await request.formData(); const firstName = clean(form.get("first_name"), 80); const lastName = clean(form.get("last_name"), 80); const email = clean(form.get("email"), 254).toLowerCase(); const confirmEmail = clean(form.get("confirm_email"), 254).toLowerCase(); const organization = clean(form.get("organization"), 160); const work = clean(form.get("work"), 4000); const stay = clean(form.get("stay"), 4000); const phone = clean(form.get("phone"), 60); const token = clean(form.get("cf-turnstile-response"), 2048);
	if (!firstName || !lastName || !email || email !== confirmEmail || !organization || !work || !stay || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json({ error: "Complete all required fields with matching valid email addresses." }, 400);
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeOrganization = escapeHtml(organization); const safeWork = escapeHtml(work).replace(/\n/g, "<br>"); const safeStay = escapeHtml(stay).replace(/\n/g, "<br>"); const safePhone = escapeHtml(phone || "Not provided"); const timestamp = new Date().toISOString(); const safeIp = escapeHtml(clean(request.headers.get("CF-Connecting-IP"), 100) || "Not available"); const safeUserAgent = escapeHtml(clean(request.headers.get("User-Agent"), 300) || "Not available"); const fromAddress = env.CONTACT_FROM_EMAIL || "noreply@3back.com";
	try {
		await sendEmail(env, { from: fromAddress, to: ["domainguides@3back.com"], reply_to: email, subject: `[Domain Guides] ${firstName} ${lastName} — ${organization}`, html: `<h1>Domain Guides request</h1><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Organization:</strong> ${safeOrganization}</p><p><strong>What is the work here?:</strong><br>${safeWork}</p><p><strong>What has to stay after outside help leaves?:</strong><br>${safeStay}</p><p><strong>Phone:</strong> ${safePhone}</p><hr><p><strong>Date:</strong> ${timestamp}</p><p><strong>IP:</strong> ${safeIp}</p><p><strong>User-Agent:</strong> ${safeUserAgent}</p>` });
		await sendEmail(env, { from: fromAddress, to: [email], subject: "We received your request – 3Back", html: "<div style=\"font-family:Arial,sans-serif;line-height:1.5;color:#1d2421\"><p>We received the request. We will see whether the program fits the domain before anything is scheduled.</p><p><strong>3Back</strong><br>The Team Execution Company</p></div>" });
