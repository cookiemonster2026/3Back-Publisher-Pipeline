...
async function handleGripCheck(request, env) {
	...
	const dimensionHtml = gripDimensions.map(([name, definition], index) => { const score = scores.dimensionScores[index]; const label = score >= 80 ? "STRONG" : score >= 50 ? "MODERATE" : "WEAK"; return `<tr><td style="padding:20px 0;border-bottom:1px solid #d8d1c6"><p style="margin:0 0 6px;font-size:18px;line-height:1.25;font-weight:700;color:#1d2421">${name}</p><p style="margin:0 0 10px;font-size:14px;line-height:1.5;color:#53605a">${definition}</p><p style="margin:0;font-size:14px;line-height:1.4;color:#1d2421"><strong style="font-size:16px">${escapeHtml(String(score))}%</strong><span style="display:inline-block;margin-left:10px;padding:3px 7px;border:1px solid #c45c26;color:#9d461b;font-size:11px;font-weight:700;letter-spacing:.08em">${label}</span></p></td></tr>`; }).join("");
	try { await sendEmail(env, { from: "gripcheck@3back.com", to: [email], subject: "Your 3Back Grip Check score", html: `<div style="margin:0;padding:0;background:#f7f3eb;font-family:Arial,sans-serif;color:#1d2421"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f7f3eb"><tr><td style="padding:32px 16px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;margin:0 auto"><tr><td style="padding:0 0 24px;border-bottom:3px solid #c45c26"><p style="margin:0;font-size:16px;line-height:1.5;color:#1d2421">Thank you. Here is your Grip Check result.</p></td></tr><tr><td style="padding:28px 0 24px"><p style="margin:0 0 6px;font-size:13px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#9d461b">Composite Grip Score</p><p style="margin:0;font-size:52px;line-height:1;font-weight:700;color:#1d2421">${safe.composite}<span style="font-size:22px;font-weight:400">/100</span></p></td></tr><tr><td style="padding:0 0 24px"><p style="margin:0;font-size:16px;line-height:1.5;color:#1d2421">This is a directional screening based on your responses. It is not a diagnostic determination.</p></td></tr><tr><td><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${dimensionHtml}</table></td></tr><tr><td style="padding:28px 0 0"><p style="margin:0;font-size:16px;line-height:1.6;color:#1d2421">This screening gives you a directional picture of where grip may be present and where it may be under strain. A fuller interpretation moves from screening toward diagnostic understanding. That requires a short conversation and a few additional questions up front so a 30-minute discussion is concrete and useful. If you want that next step, reply to this email or use <a href="https://3back.com" style="color:#1d2421;text-decoration:underline">Start a Conversation on 3back.com</a> and say you completed a Grip Check.</p></td></tr><tr><td style="padding:28px 0 0;border-top:1px solid #d8d1c6"><p style="margin:0;font-size:14px;line-height:1.6;color:#1d2421"><strong>3Back</strong><br>The Team Execution Company<br><a href="https://3back.com" style="color:#1d2421;text-decoration:underline">3back.com</a><br><a href="tel:+18553232225" style="color:#1d2421;text-decoration:underline">(855) 323-2225</a></p></td></tr></table></td></tr></table></div>` }); await sendEmail(env, { from: "gripcheck@3back.com", to: ["gripcheckscores@3back.com"], reply_to: email, subject: "[GripCheck] new result", html: `<h1>Grip Check result</h1><p><strong>View:</strong> ${safe.view}</p><p><strong>Composite:</strong> ${safe.composite}</p><p><strong>Dimensions:</strong> ${safe.dimensions}</p><p><strong>Name:</strong> ${safe.firstName} ${safe.lastName}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Position / role:</strong> ${safe.position}</p><p><strong>Organization name:</strong> ${safe.organizationName}</p><p><strong>Organization size:</strong> ${safe.organizationSize}</p><p><strong>Context:</strong><br>${safe.context.replace(/\n/g,"<br>")}</p><p><strong>30-minute interpretation and debrief requested:</strong> ${safe.reviewRequested}</p>` }); } catch { return json({ error: "We could not send your results. Please try again shortly." }, 502); }
	return json({ ok: true });
}
