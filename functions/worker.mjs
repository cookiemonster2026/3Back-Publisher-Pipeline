...
import { handleClientError, handleDigest, handleHistory, sendDigest } from "./client-error.mjs";
import { handleDeploymentHistory } from "./deployment-history.mjs";

const CONTACT_PATH = "/api/contact";
const BIO_CONTACT_PATH = "/api/bio-contact";
const CSM_PRIVATE_REQUEST_PATH = "/api/csm-private-request";
const SBK_PRIVATE_REQUEST_PATH = "/api/sbk-private-request";
const SM1_PRIVATE_INQUIRY_PATH = "/api/sm1-private-inquiry";
const SFT_PRIVATE_INQUIRY_PATH = "/api/sft-private-inquiry";
const SSWS_PRIVATE_INQUIRY_PATH = "/api/ssws-private-inquiry";
const SM2_PRIVATE_INQUIRY_PATH = "/api/sm2-private-inquiry";
const SM3_PRIVATE_INQUIRY_PATH = "/api/sm3-private-inquiry";
const PO1_PRIVATE_INQUIRY_PATH = "/api/po1-private-inquiry";
const PO2_PRIVATE_INQUIRY_PATH = "/api/po2-private-inquiry";
const PO3_PRIVATE_INQUIRY_PATH = "/api/po3-private-inquiry";
...
const GONE_PATHS = new Set([
	...
	"/courses/adaptive-team-model",
	"/courses/dysfunction-mapping-practitioner",
...
async function handleSm1PrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	const form = await request.formData(); const firstName = clean(form.get("first_name"), 80); const lastName = clean(form.get("last_name"), 80); const email = clean(form.get("email"), 254).toLowerCase(); const confirmEmail = clean(form.get("confirm_email"), 254).toLowerCase(); const companyName = clean(form.get("company_name"), 160); const note = clean(form.get("note"), 4000); const token = clean(form.get("cf-turnstile-response"), 2048);
	if (!firstName || !lastName || !email || email !== confirmEmail || !companyName || !note || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json({ error: "Complete all fields with matching valid email addresses." }, 400);
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "rsm1-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back RSM1 Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[RSM1 Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>RSM1 Private Live course inquiry</h1><p><strong>Source page:</strong> /training/scrum-mastering-1</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Scrum Team and training needs:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back RSM1 Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your RSM1 Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your Scrum Team and training needs. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handleSftPrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	const form = await request.formData(); const firstName = clean(form.get("first_name"), 80); const lastName = clean(form.get("last_name"), 80); const email = clean(form.get("email"), 254).toLowerCase(); const confirmEmail = clean(form.get("confirm_email"), 254).toLowerCase(); const companyName = clean(form.get("company_name"), 160); const note = clean(form.get("note"), 4000); const phone = clean(form.get("phone"), 60); const token = clean(form.get("cf-turnstile-response"), 2048);
	if (!firstName || !lastName || !email || email !== confirmEmail || !companyName || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json({ error: "Complete all required fields with matching valid email addresses." }, 400);
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note || "Not provided").replace(/\n/g, "<br>"); const safePhone = escapeHtml(phone || "Not provided"); const timestamp = new Date().toISOString(); const inquiryAddress = "sft-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back Scrum for Teams <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[Scrum for Teams] ${firstName} ${lastName} · ${companyName}`, html: `<h1>Scrum for Teams private course inquiry</h1><p><strong>Source page:</strong> /courses/scrum-for-teams</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Organization:</strong> ${safeCompanyName}</p><p><strong>What should we know?:</strong><br>${safeNote}</p><p><strong>Phone:</strong> ${safePhone}</p>` });
		await sendEmail(env, { from: `3Back Scrum for Teams <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your Scrum for Teams course request", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>We received the request. We will review what you shared and reply by email about whether this course fits and how to shape it.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handleSbkPrivateRequest(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course request form is not configured yet." }, 503);
	...
	if (!firstName || !lastName || !email || email !== confirmEmail || !companyName || !note || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json({ error: "Complete all fields with matching valid email addresses." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const notificationTo = env.SBK_NOTIFICATION_EMAIL || "ssbwk@3back.com";
	try { await sendEmail(env, { from: env.CONTACT_FROM_EMAIL || "noreply@3back.com", to: [notificationTo], reply_to: email, subject: `[SBK] ${firstName} ${lastName} · ${companyName}`, html: `<h1>Scrum Better with Kanban private course request</h1><p><strong>Source page:</strong> /courses/scrum-better-with-kanban</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Note:</strong><br>${safeNote}</p>` }); } catch { return json({ error: "We could not send your request. Please try again." }, 502); }
	return json({ ok: true });
}
async function handleSswsPrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	...
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "ssws-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back SSwS Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[SSwS Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>SSwS Private Live course inquiry</h1><p><strong>Source page:</strong> /training/scaling-scrum-with-scrum</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Organization and scaling problem:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back SSwS Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your SSwS Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your organization and the scaling problem. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handleSm3PrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	...
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "rsm3-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back RSM3 Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[RSM3 Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>RSM3 Private Live course inquiry</h1><p><strong>Source page:</strong> /training/scrum-mastering-3</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Scrum Team and training needs:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back RSM3 Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your RSM3 Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your Scrum Team and training needs. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handleSm2PrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	...
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "rsm2-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back RSM2 Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[RSM2 Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>RSM2 Private Live course inquiry</h1><p><strong>Source page:</strong> /training/scrum-mastering-2</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Scrum Team and training needs:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back RSM2 Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your RSM2 Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your Scrum Team and training needs. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handlePo1PrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	...
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "rpo1-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back RPO1 Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[RPO1 Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>RPO1 Private Live course inquiry</h1><p><strong>Source page:</strong> /training/product-ownership-1</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Scrum Team and training needs:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back RPO1 Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your RPO1 Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your Scrum Team and training needs. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handlePo2PrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	...
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "rpo2-private@3back.com";
	try {
		await sendEmail(env, { from: `3Back RPO2 Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[RPO2 Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>RPO2 Private Live course inquiry</h1><p><strong>Source page:</strong> /training/product-ownership-2</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Scrum Team and training needs:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back RPO2 Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your RPO2 Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your Scrum Team and training needs. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
async function handlePo3PrivateInquiry(request, env) {
	if (request.method !== "POST") return json({ error: "Method not allowed." }, 405);
	const requestUrl = new URL(request.url); const origin = request.headers.get("origin");
	if (origin && origin !== requestUrl.origin) return json({ error: "Invalid request origin." }, 403);
	if (!env.TURNSTILE_SECRET_KEY || !env.RESEND_API_KEY) return json({ error: "The private course inquiry form is not configured yet." }, 503);
	...
	if (!token) return json({ error: "Please complete the security check and try again." }, 400);
	if (!await verifyTurnstile(request, env, token)) return json({ error: "The security check could not be verified. Please try again." }, 400);
	const safeFirstName = escapeHtml(firstName); const safeLastName = escapeHtml(lastName); const safeEmail = escapeHtml(email); const safeCompanyName = escapeHtml(companyName); const safeNote = escapeHtml(note).replace(/\n/g, "<br>"); const timestamp = new Date().toISOString(); const inquiryAddress = "po3@3back.com";
	try {
		await sendEmail(env, { from: `3Back RPO3 Private Live <${inquiryAddress}>`, to: [inquiryAddress], reply_to: email, subject: `[RPO3 Private Live] ${firstName} ${lastName} · ${companyName}`, html: `<h1>RPO3 Private Live course inquiry</h1><p><strong>Source page:</strong> /training/product-ownership-3</p><p><strong>Timestamp:</strong> ${timestamp}</p><p><strong>First name:</strong> ${safeFirstName}</p><p><strong>Last name:</strong> ${safeLastName}</p><p><strong>Email:</strong> ${safeEmail}</p><p><strong>Company name:</strong> ${safeCompanyName}</p><p><strong>Scrum Team and training needs:</strong><br>${safeNote}</p>` });
		await sendEmail(env, { from: `3Back RPO3 Private Live <${inquiryAddress}>`, to: [email], reply_to: inquiryAddress, subject: "We received your RPO3 Private Live course inquiry", html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1d2421"><p>Thank you for contacting 3Back. We received your inquiry. We'll reply by email to schedule a 15- to 30-minute conversation about your Scrum Team and training needs. You can reply directly to this email if there is anything else you would like us to know.</p><p><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:${inquiryAddress}" style="color:#1d2421">${inquiryAddress}</a></p></div>` });
	} catch { return json({ error: "We could not send your inquiry. Please try again." }, 502); }
	return json({ ok: true });
}
...
async function buildGripCheckBankAttachment(request, env) {
  const bytes = await fetchPublicPdf(request, env, "/assets/grip-check/3Back-Grip-Check-Question-Bank.pdf");
  return { filename: "3Back-Grip-Check-Question-Bank.pdf", content: arrayBufferToBase64(bytes), content_type: "application/pdf" };
}
