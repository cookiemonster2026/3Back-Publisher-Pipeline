...
function buildGripCheckBankAttachment() {
	...
	const items = questions.map((item, index) => {
		if (typeof item.question !== "string" || !item.question.trim() || !Array.isArray(item.options) || item.options.length < 4 || item.options.some(option => typeof option !== "string" || !option.trim())) throw new Error("Invalid Grip Check bank item.");
		const options = item.options.map((option, optionIndex) => `<li><strong>Option index ${optionIndex}: ${Math.max(0, 3 - optionIndex)} points.</strong> ${escapeHtml(option)}</li>`).join("");
		return `<section><p class="dimension">${escapeHtml(dimensions[Math.floor(index / 4)])}</p><h2>${index + 1}. ${escapeHtml(item.question)}</h2><ol>${options}</ol></section>`;
	}).join("\n");
	const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Grip Check Question Bank</title><style>body{max-width:900px;margin:40px auto;padding:0 24px;font:16px/1.5 Arial,sans-serif;color:#1d2421}h1{font-size:28px}h2{font-size:20px}.dimension{font-weight:bold;margin-bottom:8px}section{border-top:1px solid #d8d1c6;margin-top:28px;padding-top:16px;break-inside:avoid}li{margin:8px 0}</style></head><body><h1>Grip Check Question Bank</h1><p>Internal use only</p><p>Generated from src/data/grip-check-questions.js</p><h2>Scoring rules</h2><ul><li>Option index 0 = 3, 1 = 2, 2 = 1, 3+ = 0</li><li>Composite = round(sum / (n × 3) × 100)</li><li>Five dimensions, four items each, source order</li><li>Dimension score = round(sum in dimension / (count × 3) × 100)</li><li>80+ STRONG, 50–79 MODERATE, under 50 WEAK</li></ul>${items}</body></html>`;
	return { filename: "3Back-Grip-Check-Question-Bank.html", content: arrayBufferToBase64(new TextEncoder().encode(html)), content_type: "text/html" };
}
