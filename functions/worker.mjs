...
import handsBar from "../public/assets/operational-grip/hands-bar.png";
...
async function handleGripCheck(request, env) {
	...
	const reviewStatus = reviewRequested === "yes" ? "Response required: 30-minute interpretation and debrief requested" : "No response required";
	...
	const dimensionSummaryHtml = gripDimensions.map(([name], index) => `<p><strong>${name}:</strong> ${escapeHtml(String(scores.dimensionScores[index]))}</p>`).join("");
	try { await sendEmail(env, { from: "gripcheck@3back.com", to: [email], subject: "Your 3Back Grip Check score", html: `<div style="margin:0;padding:0;background:#f7f3eb;font-family:Arial,sans-serif;color:#1d2421"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f7f3eb"><tr><td style="padding:20px 12px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;margin:0 auto"><tr><td style="padding:0 0 16px;border-bottom:3px solid #c45c26"><p style="margin:0;font-size:16px;line-height:1.5;color:#1d2421">Thank you. Here is your Grip Check result for ${perspective}.</p></td></tr><tr><td style="padding:20px 0 16px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td><p style="margin:0 0 6px;font-size:13px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#9d461b">Composite Grip Score for ${perspective}</p><p style="margin:0;font-size:52px;line-height:1;font-weight:700;color:#1d2421">${safe.composite}<span style="font-size:22px;font-weight:400">/100</span></p></td><td width="130" style="padding-left:16px;text-align:right;vertical-align:middle"><img src="cid:hands-bar" alt="" width="120" style="display:inline-block;width:120px;height:auto;border:0"></td></tr></table></td></tr><tr><td><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${dimensionHtml}</table></td></tr><tr><td style="padding:18px 0 0"><p style="margin:0;font-size:16px;line-height:1.6;color:#1d2421">This was your screening, not a diagnostic determination. A fuller interpretation needs a short conversation. Reply to this email or use <a href="https://3back.com" style="color:#1d2421;text-decoration:underline">Start a Conversation on 3back.com</a> and say you completed a Grip Check. We will reply with a few short questions and available times.</p></td></tr><tr><td style="padding:20px 0 0;border-top:1px solid #d8d1c6"><p style="margin:0;font-size:14px;line-height:1.6;color:#1d2421"><strong>3Back</strong><br>The Team Execution Company<br><a href="mailto:gripcheck@3back.com" style="color:#1d2421;text-decoration:underline">gripcheck@3back.com</a><br><a href="https://3back.com" style="color:#1d2421;text-decoration:underline">3back.com</a><br><a href="tel:+18553232225" style="color:#1d2421;text-decoration:underline">(855) 323-2225</a></p></td></tr></table></td></tr></table></div>`, attachments: [{ filename: "hands-bar.png", content: arrayBufferToBase64(handsBar), content_type: "image/png", content_id: "hands-bar" }] }); await sendEmail(env, { from: "gripcheck@3back.com", to: ["gripcheckscores@3back.com"], reply_to: email, subject: `[GripCheck] ${safe.firstName} ${safe.lastName} · ${safe.organizationName} · ${safe.composite}`, html: `<h1>Grip Check result</h1><p><strong>${reviewStatus}</strong></p><p><strong>Perspective:</strong> ${internalPerspective}</p><p><strong>Composite:</strong> ${safe.composite}</p>${dimensionSummaryHtml}<p><strong>Name:</strong> ${safe.firstName} ${safe.lastName}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Position / role:</strong> ${safe.position}</p><p><strong>Organization name:</strong> ${safe.organizationName}</p><p><strong>Organization size:</strong> ${safe.organizationSize}</p><p><strong>Context:</strong><br>${safe.context.replace(/\n/g,"<br>")}</p><p><strong>30-minute interpretation and debrief requested:</strong> ${safe.reviewRequested}</p>` }); } catch { return json({ error: "We could not send your results. Please try again shortly." }, 502); }
	return json({ ok: true });
}
