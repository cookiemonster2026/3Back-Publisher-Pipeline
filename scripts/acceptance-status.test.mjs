...
test('the complete baseline has 92 active live, eight process, and five retired items', () => {
  assert.equal(LIVE_ITEMS.length, 92);
  ...
  assert.equal(result.applicable, 92);
  assert.equal(result.processCount, 8);
  assert.equal(result.retiredCount, 5);
  assert.equal(result.counts.unverified, 92);
