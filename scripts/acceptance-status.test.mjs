...
test('the complete baseline has exactly 81 live and eight process items', () => {
  assert.equal(LIVE_ITEMS.length, 81);
  ...
  assert.equal(result.applicable, 81);
  assert.equal(result.processCount, 8);
  assert.equal(result.counts.unverified, 81);
