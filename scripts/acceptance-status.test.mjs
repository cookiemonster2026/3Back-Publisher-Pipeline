...
test('the complete baseline has 102 active live, nine process, and five retired items', () => {
  assert.equal(LIVE_ITEMS.length, 102);
  ...
  assert.equal(result.applicable, 102);
  assert.equal(result.processCount, 9);
  assert.equal(result.retiredCount, 5);
  assert.equal(result.counts.unverified, 102);
