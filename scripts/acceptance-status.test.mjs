...
test('the complete baseline has 100 active live, nine process, and five retired items', () => {
  assert.equal(LIVE_ITEMS.length, 100);
  ...
  assert.equal(result.applicable, 100);
  assert.equal(result.processCount, 9);
  assert.equal(result.retiredCount, 5);
  assert.equal(result.counts.unverified, 100);
...
test('retirement removes current scoring but preserves historical evidence and archives', () => {
  ...
  assert.ok(!current.html.includes('id="704"'));
