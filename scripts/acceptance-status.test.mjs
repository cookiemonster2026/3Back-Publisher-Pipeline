...
test('the complete baseline has exactly 77 live and eight process items', () => {
  assert.equal(LIVE_ITEMS.length, 77);
  ...
  assert.equal(result.applicable, 77);
  assert.equal(result.processCount, 8);
  assert.equal(result.counts.unverified, 77);
