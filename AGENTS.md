...
Acceptance snapshot milestone dates: Record builtAt at implementation completion, reviewedAt when the independent reviewer completes the review, and judgedAt only when the human explicitly accepts the results and closes the task. Capture the actual event time as an ISO timestamp as part of performing that step; do not ask the human to supply the clock time. Never infer these milestones from a push, individual test timestamps, task opening, or a local check. Leave unavailable dates null (displayed as Not Recorded). Set each milestone once; preserve earlier snapshot evidence and history. These fields are populated by the agent workflow, not by a background event listener.
