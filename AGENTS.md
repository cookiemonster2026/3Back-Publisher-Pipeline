...
Before a requested release push, ensure the completed work has its own snapshot and linked Done milestone. Freeze the suite and current evidence together; carry forward unaffected results without claiming a new review. Append entries, never overwrite older snapshots. Every push handoff must include the production URL for that work's acceptance results, the GitHub changes link, and whether production deployment was verified or remains pending. A successful push alone is not deployment evidence.
