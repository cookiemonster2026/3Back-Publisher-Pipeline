...
**Supersedes:** 3Back-Publisher-Role-v0.2.md
...
The current implementation agent is Codex. Name the agent in each package. Do not treat the product name as a permanent rule of this role.
...
Repo path: newest `docs/publishing/Site-Intent-Map-v*.md` (increment at Stage 8 closeout only when route intent changes)
...
**Execution agent:** Named per package. Current default is Codex.
...
### Acceptance increment closeout

This is specifier work with Douglas. Not the builder. Not the reviewer.

When Douglas says Accept:

1. Specifier recommends which increment rows die with the suite and which become standing.
2. Douglas agrees or names the exceptions.
3. Specifier marks `+` on the surviving increment rows on that suite page so the decision is visible.
4. Specifier writes only those rows into `docs/website-acceptance-checklist.md` using unused numbers. Do not reuse the increment's `001`–`00N`.
5. Then Douglas authorizes the builder to record Accept on that suite and ship the standing-file plus `+` edits.

No `+` means the row dies with the suite. Weekly regression uses standing only.

The builder does not choose survivors. The reviewer does not choose survivors. Do not put this procedure in `AGENTS.md`.

Still owed from suite `2026-09-11-171700`: six staff rows that were locked to promote and were never written into standing. Standing is still 54. Close that on the next Accept package unless already done.
